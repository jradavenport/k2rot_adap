...
Of course, this relies on the rotational and kinematic age relations being
correct.

As discussed above, the performance of the age-rotation relations for old
stars is questionable.
...
stars.
This hypothesis is supported by the data, however the data are spars~---~this
analysis was conducted on the small number of Main sequence, Solar-like
oscillators observed by Kepler in short cadence mode with detectable rotation
periods: a sample size of around 26.
Furthermore, asteroseismology favors stars with low surface gravities since
these stars show the largest amplitudes of oscillation and the majority of the
stars in this sample were slightly evolved.
A larger sample of old main sequence stars with precise ages is required to
confirm and further characterise the Rossby saturation mechanism currently
supported by the Kepler data but of course old main sequence stars are
difficult to age-date with any other method.
We propose use the age gradients in the kinematic properties of stars to
confirm the Rossby saturation effect.

The age-velocity dispersion relations are relatively well studied and simple.
The current picture of the formation history of stars in the Milky Way is that
they are formed in the thin disc with relatively small vertical velocities and
actions.
As time passes these stars encounter more scattering events~---~close
encounters with other stars, and/or interactions with galactic spiral arms.
With each of these interactions the star's orbit in the Milky Way is excited.
This results in a slow heating of stellar orbits.
Old stars reside in the thick disc of the Milky Way~---~a disc that extends
out of the plane of the galaxy.
These stars can be identified in Gaia DR1 by converting their proper motions,
positions and parallaxes into vertical actions by integrating their orbits in
the potential of the Milky Way.
Larger vertical actions (larger angular momenta in the vertical direction)
correspond to older stars.
Truely the {\it dispersion} in vertical action is a much better tracer of age
than vertical action itself but for single stars, only the individual vertical
action, or $J_z^2$ is available.
Nonetheless, $J_z^2$ still increases over time, it just provides a relatively
weak constraint on age.
...
braking at zero-age main sequence and the transition scenario in which stars
stop braking at a critical Rossby number.
...
by APOGEE with ages from the Cannon \citep{Ness2015}, also observed by Gaia.
